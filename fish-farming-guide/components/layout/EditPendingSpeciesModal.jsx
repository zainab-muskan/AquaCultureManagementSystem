...
export default function EditPendingSpeciesModal({ isOpen, onClose, species, onApprove, isApprovedMode }) {
    ...
    useEffect(() => {
        if (isOpen && species) {
            const fetchData = async () => {
                setLoadingData(true);
                try {
                    const [regionsData, speciesData, compatibilityData] = await Promise.all([
                        farmApi.getRegions(),
                        farmApi.getApprovedSpecies(),
                        farmApi.getSpeciesCompatibility(species.SpeciesId).catch(() => [])
                    ]);
                    setRegions(regionsData || []);
                    setAllSpecies(speciesData || []);
                    
                    const compRegionsStr = species.CompatibleRegions || "";
                    if (compRegionsStr && regionsData) {
                        const matched = regionsData.filter(r => compRegionsStr.includes(r.Name)).map(r => r.Name);
                        setSelectedRegions(matched);
                    } else {
                        setSelectedRegions([]);
                    }
                    
                    if (compatibilityData && compatibilityData.length > 0) {
                        // Compatibility data has CompatibleSpeciesName, finding IDs might require matching or backend might return IDs
                        // Let's just fetch IDs from the speciesData based on the names, or if the backend returns the IDs, use them.
                        // Actually, looking at speciesRoutes.js getSpeciesCompatibility, it doesn't return CompatibleWithId cleanly.
                        // It returns MainSpeciesName and CompatibleSpeciesName.
                    }
                    // A better simpler approach is to select them by matching name if backend doesn't return ID.
                    const compIds = compatibilityData.map(c => {
                        const sName = c.CompatibleSpeciesName === species.Name ? c.MainSpeciesName : c.CompatibleSpeciesName;
                        const matchingS = (speciesData || []).find(s => s.Name === sName);
                        return matchingS ? matchingS.SpeciesId : null;
                    }).filter(Boolean);
                    
                    setSelectedCompatibleSpeciesIds(compIds);
                } catch (err) {
    ...
    useEffect(() => {
        if (species) {
            setFormData({
                ...
                MaxStockingDensity: species.MaxStockingDensity || "",
                CompatibleRegions: species.CompatibleRegions || "",
                MinTemp: species.MinTemp || "",
                MaxTemp: species.MaxTemp || "",
                MinPH: species.MinPH || "",
                MaxPH: species.MaxPH || "",
                MinDO: species.MinDO || "",
                FingerlingSizeG: species.FingerlingSizeG || "",
                MarketSizeKG: species.MarketSizeKG || "",
                HarvestTimeMonths: species.HarvestTimeMonths || "",
                SurvivalRateLower: species.SurvivalRateLower || "",
                SurvivalRateUpper: species.SurvivalRateUpper || "",
                MinMarketPrice: species.MinMarketPrice || "",
                MaxMarketPrice: species.MaxMarketPrice || "",
                Description: species.Description || "",
                FeedingZone: species.FeedingZone || "",
            });
        }
    }, [species]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await onApprove(species.SpeciesId, submissionData);
            onClose();
        } catch (err) {
            setError(err.message || "Failed to update and approve species");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 overflow-y-auto">
            <div className="bg-white w-full max-w-2xl flex flex-col max-h-[90vh] sm:max-h-[85vh] rounded-[1.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                {/* Header */}
                <div className="flex justify-between items-start px-8 pt-8 pb-4 border-b border-gray-100">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">{isApprovedMode ? "Edit Species" : "Review & Approve Species"}</h2>
                        <p className="text-sm text-gray-500 mt-1">{isApprovedMode ? "Make any necessary changes to this published species." : "Review the submitted details, make any necessary edits, and approve."}</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="px-8 pb-8 pt-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
                    {error && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                        ...
                        <div className="col-span-1 md:col-span-2 space-y-1.5">
                            ...
                            <input required name="Name" value={formData.Name} onChange={handleChange} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm font-medium text-gray-900 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-gray-300" />
                        </div>

                        <div className="col-span-1 md:col-span-2 space-y-2">
                            <label className="text-[13px] font-bold text-gray-700 ml-1">Compatible Regions *</label>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input required type="number" name="MaxStockingDensity" value={formData.MaxStockingDensity} onChange={handleChange} placeholder="4000" className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm font-medium text-gray-900 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-gray-300" />
                        </div>
                        ...
                        <div className="col-span-1 md:col-span-2 space-y-2">
                            ...
                            {loadingData ? (
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                    {allSpecies.map((s) => (
                                        <label key={s.SpeciesId} className="flex items-center gap-2 cursor-pointer group">
                                            <div className="relative flex items-center">
                                                <input
                                                    type="checkbox"
                                                    checked={selectedCompatibleSpeciesIds.includes(s.SpeciesId)}
                                                    onChange={() => handleCompatibleSpeciesChange(s.SpeciesId)}
                                                    className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 transition-all cursor-pointer"
                                                />
                                            </div>
                                            <span className={`text-sm font-medium transition-colors ${selectedCompatibleSpeciesIds.includes(s.SpeciesId) ? "text-blue-700" : "text-gray-900 group-hover:text-gray-800"}`}>
                                                {s.Name}
                                            </span>
                                        </label>
                                    ))}
                    ...
                    <div className="flex gap-4 pt-4 border-t border-gray-100 mt-6">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="flex-1 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-200 transition-all flex items-center justify-center gap-2 group/btn"
                        >
                            {loading ? (
                                <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                            ) : (
                                <>
                                    {isApprovedMode ? <Save size={18} strokeWidth={2.5} className="group-hover/btn:scale-110 transition-transform" /> : <CheckCircle2 size={18} strokeWidth={2.5} className="group-hover/btn:scale-110 transition-transform" />}
                                    {isApprovedMode ? "Save Changes" : "Save & Approve Species"}
                                </>
                            )}
                        </button>
                    </div>
