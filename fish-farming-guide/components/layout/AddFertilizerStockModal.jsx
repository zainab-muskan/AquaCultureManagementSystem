...
export default function AddFertilizerStockModal({ isOpen, onClose, onSuccess }) {
    ...
    // Fetch dynamic fertilizer products on open
    useEffect(() => {
        if (isOpen) {
    ...
    const fetchFertilizerProducts = async () => {
        try {
            const products = await farmApi.getFertilizerProducts();
            setFertilizerOptions(products || []);
        } catch (err) {
            console.error("Could not fetch dynamic fertilizer products:", err);
    ...
    const availableProducts = useMemo(() => {
        return fertilizerOptions
            .filter(opt => opt.Category === formData.category)
            .map(opt => opt.Product);
    }, [fertilizerOptions, formData.category]);
    ...
    const handleSubmit = async (e) => {
        ...
        const finalProduct = formData.productName === "Other" ? formData.customProductName : formData.productName;

        if (!finalProduct || finalProduct.trim() === "") {
            setError("Please select or enter a valid Product Name.");
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            setError(err.message || "Failed to add fertilizer stock");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-0">
            ...
            <div className="relative w-full max-w-lg bg-white flex flex-col max-h-[90vh] sm:max-h-[85vh] rounded-[24px] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 shrink-0">
                    <div>
                        <h2 className="text-xl font-black text-slate-900">Add Fertilizer Stock</h2>
                        <p className="text-xs text-slate-500 font-medium mt-1">Record a new fertilizer purchase</p>
                    </div>
                    ...
                    <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                        ...
                        <div className="space-y-5">
                            <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => {
                                            setFormData({
                                                ...
                                                productName: "" // Reset product on category change
                                            });
                                        }}
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm font-medium outline-none transition-all"
                                    >
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-black text-slate-900 uppercase tracking-widest">Product Name</label>
                                    <select
                                        name="productName"
                                        value={formData.productName}
                                        onChange={handleChange}
                                        required
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm font-medium outline-none transition-all bg-white opacity-100 relative z-20"
                                    >
                            ...
                            {formData.productName === "Other" && (
                                <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2">
                                    <label className="text-xs font-black text-slate-900 uppercase tracking-widest">Custom Product Name</label>
                                    <input
                                        type="text"
                            ...
                            <div className="p-4 bg-emerald-50/50 rounded-xl border border-emerald-100 flex justify-between items-center">
                                <span className="text-xs font-black text-emerald-900 uppercase tracking-widest">Total Investment</span>
